...
final class EcgWaveGenerator {
    ...
    EcgSourceSample generateEcgValue(final UUID patientId) {
        ...
        if (shouldInjectCriticalSpike(patientId, currentTimeMillis)) {
            final AtomicLong criticalSpikeMillis = lastOccurredSpikes.get(patientId);
            if (criticalSpikeMillis == null) {
                lastOccurredSpikes.put(patientId, new AtomicLong(currentTimeMillis));
            } else {
                criticalSpikeMillis.set(currentTimeMillis);
            }
            return EcgSourceSample.of(patientId, bound(transformValueToSpike(valueWithNoise)));
        }
        return EcgSourceSample.of(patientId, bound(valueWithNoise));
    }
    ...
    private boolean shouldInjectCriticalSpike(final UUID id, final long currentTimeMillis) {
        final AtomicLong criticalSpikeMillis = lastOccurredSpikes.get(id);
        if (criticalSpikeMillis == null) {
            return true;
        }
    ...
    private double generatePlainEcgValue(final long currentTimeMillis) {
    ...
    private double gaussianPulse(double secondsSinceEpoch, double center, double width, double amplitude) {
